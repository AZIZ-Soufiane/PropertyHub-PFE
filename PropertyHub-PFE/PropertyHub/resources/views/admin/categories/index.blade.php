...
@section('page-subtitle', 'Manage property types and categories')
...
<div x-data="categoryManager()">
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-slate-500 font-semibold">{{ $categories->count() }} categories</p>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($categories as $category)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <code class="text-xs text-slate-400 font-mono bg-slate-100 px-2 py-1 rounded-lg">{{ $category->slug }}</code>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="flex justify-end items-center gap-2">
                                    <button type="button" @click="openEditModal({{ $category->id }}, '{{ addslashes($category->name) }}', {{ $category->is_active ? 'true' : 'false' }})"
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0 lg:ps-64">
            <div x-show="showModal"
                ...
                <form :action="editing ? `/admin/categories/${editId}` : '{{ route('admin.categories.store') }}'"
