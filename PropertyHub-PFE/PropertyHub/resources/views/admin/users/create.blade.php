...
@section('title', isset($user) && $user->exists ? 'Edit User' : 'New User')
@section('page-title', isset($user) && $user->exists ? 'Edit user' : 'Create new user')
@section('page-subtitle', isset($user) && $user->exists ? 'Update account information and permissions.' : 'Add a new member to the platform.')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-3xl shadow-soft p-6">
                <div class="flex items-center gap-3 mb-5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">{{ $isEdit ? 'Leave blank to keep current password.' : 'Choose a strong password.' }}</p>
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block mb-2 text-xs font-bold text-slate-700 uppercase tracking-wide">Password {{ $isEdit ? '(optional)' : '*' }}</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="role" required
                                class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none transition-all">
                            <option value="buyer"  @selected($cur === 'buyer')>Buyer</option>
                            <option value="agent"  @selected($cur === 'agent')>Agent</option>
                            <option value="admin"  @selected($cur === 'admin')>Admin</option>
                        </select>
                    </div>
                    <div>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.users.index') }}" class="py-3 px-6 text-sm font-bold rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 transition-all">Cancel</a>
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <a href="{{ route('admin.users.index') }}" class="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-primary-600 transition-colors">
