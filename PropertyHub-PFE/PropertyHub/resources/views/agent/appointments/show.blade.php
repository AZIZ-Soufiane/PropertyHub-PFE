...
<div class="max-w-4xl mx-auto space-y-6">
    <a href="{{ route('agent.appointments.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-700 transition-colors">
    ...
    <div class="bg-white border border-gray-200 shadow-sm rounded-3xl overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
            ...
            <span class="py-1 px-3 inline-flex items-center gap-1 rounded-full text-[10px] font-bold uppercase {{ $pill }}">
                <span class="size-1.5 rounded-full {{ $dot }}"></span>{{ ucfirst($st) }}
            </span>
        </div>
            ...
            <div class="space-y-5">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm font-bold text-gray-800 mt-0.5">{{ optional($appointment->client)->name ?? 'Unknown' }}</p>
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Email</label>
                        <p class="text-sm text-gray-600 mt-0.5">{{ optional($appointment->client)->email ?? '—' }}</p>
                    </div>
                    @if(optional($appointment->client)->phone)
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Phone</label>
                        <p class="text-sm text-gray-600 mt-0.5">{{ $appointment->client->phone }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="space-y-5">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm font-bold text-gray-800 mt-0.5">{{ optional($appointment->property)->title ?? 'Not specified' }}</p>
                    </div>
                    @if(optional($appointment->property)->city)
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Location</label>
                        <p class="text-sm text-gray-600 mt-0.5">{{ $appointment->property->city }}{{ $appointment->property->address ? ', ' . $appointment->property->address : '' }}</p>
                    </div>
                    @endif
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Price</label>
                        <p class="text-sm font-bold text-primary-600 mt-0.5">{{ optional($appointment->property)->price ? 'DH ' . number_format($appointment->property->price, 0, ',', ' ') : '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-5">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm font-bold text-gray-800 mt-0.5">{{ $appointment->date_time->format('l, F d, Y') }}</p>
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Time</label>
                        <p class="text-sm text-gray-600 mt-0.5">{{ $appointment->date_time->format('h:i A') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white border border-gray-200 shadow-sm rounded-3xl overflow-hidden">
        ...
        <div class="p-6 flex items-center gap-3">
            <form action="{{ route('agent.appointments.confirm', $appointment) }}" method="POST">
                @csrf
