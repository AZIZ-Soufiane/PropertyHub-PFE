        ...
        <div class="overflow-y-auto flex-1">
            @forelse($conversations as $conv)
                ...
                <a href="{{ route('agent.messages.show', $other) }}"
                    class="flex items-center gap-x-4 p-4 hover:bg-gray-50 transition-all {{ request()->route('agent.messages.show', $other) ? 'bg-white border-r-4 border-primary-500' : '' }}">
                    ...
                    <div class="grow min-w-0">
                        ...
                        <p class="text-xs text-gray-500 truncate font-medium">{{ $conv->content }}</p>
                    </div>
                    <span class="text-[10px] text-gray-400 font-bold whitespace-nowrap">{{ $conv->created_at->diffForHumans(null, true) }}</span>
                </a>
            @empty
