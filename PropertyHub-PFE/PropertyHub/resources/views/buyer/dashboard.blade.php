...
<div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5">
        ...
        <div class="flex items-center gap-x-2">
            <h3 class="text-3xl font-black text-gray-800">{{ $stats['confirmed'] }}</h3>
            <span class="text-xs font-bold py-1 px-2 bg-emerald-100 text-emerald-800 rounded-lg">Confirmed</span>
        </div>
    </div>
    <a href="{{ route('buyer.favorites.index') }}" class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:border-rose-200 hover:bg-rose-50/30 transition-all group">
        ...
        <div class="flex items-center gap-x-2">
            <h3 class="text-3xl font-black text-gray-800">{{ $stats['favorites'] }}</h3>
...
<div class="grid lg:grid-cols-2 gap-8">
    ...
    <div class="space-y-4">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('buyer.appointments.index') }}" class="text-sm font-semibold text-primary-600 hover:text-primary-700">View All</a>
        </div>
        @forelse($appointments as $appt)
            ...
            <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:border-primary-300 transition-all">
                <div class="flex justify-between items-start mb-3">
                    <div class="space-y-1">
                        <h3 class="font-bold text-gray-800">{{ $appt->date_time->format('D, M d, Y') }}</h3>
                        <p class="text-sm text-gray-500">{{ $appt->date_time->format('H:i') }}</p>
                    </div>
    ...
    <div class="space-y-4">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('buyer.messages.index') }}" class="text-sm font-semibold text-primary-600 hover:text-primary-700">Open Inbox</a>
        </div>
