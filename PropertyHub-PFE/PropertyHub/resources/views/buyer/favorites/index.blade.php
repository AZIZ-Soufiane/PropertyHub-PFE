...
@if($favorites->isEmpty())
    <div class="bg-white border border-gray-200 rounded-2xl p-16 text-center">
        ...
        <a href="{{ route('properties.index') }}" class="mt-6 inline-flex items-center gap-2 py-2.5 px-5 bg-primary-600 text-white rounded-xl font-bold text-sm hover:bg-primary-700 transition-all">
            Browse Properties
        </a>
    </div>
@else
        ...
        @foreach($favorites as $property)
            ...
            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all group">
                <div class="relative h-48 overflow-hidden">
                    ...
                    <span class="absolute top-3 left-3 py-1 px-3 rounded-full text-[10px] font-bold {{ $pill }} uppercase">
                        {{ ucfirst($statusName) }}
                    </span>
                    {{-- Remove from favorites --}}
                    <form action="{{ route('buyer.favorites.toggle', $property) }}" method="POST" class="absolute top-3 right-3">
                        @csrf
                ...
                <div class="p-5">
                    <p class="text-xs font-bold uppercase tracking-wider text-primary-500 mb-1">{{ ucfirst($property->type) }}</p>
                    <h3 class="text-base font-bold text-gray-800 mb-1 truncate">{{ $property->title }}</h3>
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-lg font-black text-gray-900">${{ number_format($property->price) }}</span>
                        <div class="flex items-center gap-3 text-xs text-gray-500 font-medium">
                            ...
                            @endif
                            @if($property->area)
                                <span>{{ number_format($property->area) }} ft²</span>
                            @endif
                        </div>
                    </div>
