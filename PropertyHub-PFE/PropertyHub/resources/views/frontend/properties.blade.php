                            ...
                            <div class="absolute top-6 right-6">
                                @auth
                                    @if(auth()->user()->role === 'buyer')
                                        @php $isFav = auth()->user()->favorites()->where('property_id', $property->id)->exists(); @endphp
                                        <form action="{{ route('buyer.favorites.toggle', $property) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                                class="p-2.5 backdrop-blur-sm rounded-full shadow-sm transition-all {{ $isFav ? 'bg-rose-500 text-white hover:bg-rose-600' : 'bg-white/90 text-gray-400 hover:text-rose-500' }}">
