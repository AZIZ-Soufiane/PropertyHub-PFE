                    ...
                    <div class="hs-dropdown-menu transition-[opacity,margin] hs-dropdown-open:opacity-100 opacity-0 hidden w-80 bg-white shadow-xl rounded-2xl border border-slate-100 mt-2 z-50 overflow-hidden"
                        ...
                        <div class="divide-y divide-slate-50 max-h-72 overflow-y-auto">
                            @forelse(auth()->user()->notifications()->take(5)->get() as $notification)
                                <div class="flex items-start gap-3 px-4 py-3 {{ $notification->read_at ? 'opacity-60' : 'hover:bg-slate-50 transition-colors' }}">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-slate-800">{{ $notification->data['title'] ?? 'Notification' }}</p>
                                        <p class="text-xs text-slate-400 mt-0.5">{{ $notification->data['message'] ?? '' }}</p>
                                        <p class="text-[10px] text-slate-300 font-semibold mt-1">{{ $notification->created_at->diffForHumans() }}</p>
                                    </div>
