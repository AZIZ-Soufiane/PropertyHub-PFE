...
<main class="pt-20">
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div x-data="propertySlider(<?php echo \Illuminate\Support\Js::from($property->all_image_urls)->toHtml() ?>)" @keydown.arrow-right.window="next()" @keydown.arrow-left.window="prev()" class="mb-8">
            <!-- Main Image -->
            ...
            <div class="w-full lg:w-1/3">
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 sticky top-24">
                    ...
                    <form action="<?php echo e(route('appointments.store')); ?>" method="POST" class="space-y-4"
                          x-data="visitScheduler(<?php echo e($property->id); ?>, '<?php echo e(route('appointments.available-slots')); ?>')">
                        <?php echo csrf_field(); ?>
                        ...
                        <div>
                            ...
                            <select name="time_slot" required class="w-full py-3 px-4 border border-gray-200 rounded-xl text-sm">
                                ...
                                <template x-for="slot in slots" :key="slot">
                                    <option :value="slot" x-text="slot + ':00'"></option>
                                </template>
                                <option value="" disabled x-show="!loading && slots.length === 0 && date">No available slots</option>
                            </select>
