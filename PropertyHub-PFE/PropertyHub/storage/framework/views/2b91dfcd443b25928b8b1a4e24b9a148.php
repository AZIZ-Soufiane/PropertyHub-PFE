<?php $__env->startSection('title', 'Edit ' . $property->title); ?>
<?php $__env->startSection('page-title', 'Edit property'); ?>
<?php $__env->startSection('page-subtitle', $property->title); ?>

<?php $__env->startSection('content'); ?>
<div class="bg-white border border-slate-200 rounded-3xl overflow-hidden">
    <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
        <div>
            <h2 class="text-lg font-black text-slate-800"><?php echo e($property->title); ?></h2>
            <p class="text-xs text-slate-400 font-semibold">Created <?php echo e($property->created_at->format('M d, Y')); ?></p>
        </div>
    ...
    <form action="<?php echo e(route('admin.properties.update', $property)); ?>" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="sm:col-span-2">
                ...
                <input type="text" name="title" required value="<?php echo e(old('title', $property->title)); ?>"
            ...
            <div>
                ...
                <input type="number" name="price" required step="0.01" value="<?php echo e(old('price', $property->price)); ?>"
                       class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 outline-none transition-all">
            </div>
            ...
            <div>
                ...
                <input type="number" name="bedrooms" min="0" value="<?php echo e(old('bedrooms', $property->bedrooms)); ?>"
                       class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 outline-none transition-all">
            </div>

            <div>
                <label class="block mb-2 text-xs font-bold text-slate-800 uppercase tracking-wide">Bathrooms</label>
                <input type="number" name="bathrooms" min="0" value="<?php echo e(old('bathrooms', $property->bathrooms)); ?>"
                       class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 outline-none transition-all">
            </div>

            <div>
                <label class="block mb-2 text-xs font-bold text-slate-800 uppercase tracking-wide">Area (sq ft)</label>
                <input type="number" name="area" min="0" step="0.01" value="<?php echo e(old('area', $property->area)); ?>"
                       class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 outline-none transition-all">
            </div>

            <div>
                <label class="block mb-2 text-xs font-bold text-slate-800 uppercase tracking-wide">City</label>
                <input type="text" name="city" value="<?php echo e(old('city', $property->city)); ?>"
                       class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 outline-none transition-all">
            </div>

            <div>
                <label class="block mb-2 text-xs font-bold text-slate-800 uppercase tracking-wide">Country</label>
                <input type="text" name="country" value="<?php echo e(old('country', $property->country)); ?>"
                       class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 outline-none transition-all">
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="description" rows="4"
                          class="py-3 px-4 block w-full border border-slate-200 rounded-xl text-sm focus:border-primary-500 focus:ring-primary-500 outline-none transition-all"><?php echo e(old('description', $property->description)); ?></textarea>
            </div>
                ...
                <?php if($property->images->count() > 0): ?>
                    <div class="mt-4 grid grid-cols-4 sm:grid-cols-6 gap-2">
                        <?php $__currentLoopData = $property->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php $urls = is_array($img->image_urls) ? $img->image_urls : [$img->image_urls]; ?>
                            <?php $__currentLoopData = $urls; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $url): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <img src="<?php echo e($url); ?>" class="w-full h-20 object-cover rounded-lg border border-slate-100">
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                <?php endif; ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\GitHub\PropertyHub-PFE\PropertyHub-PFE\PropertyHub\resources\views\admin\properties\edit.blade.php ENDPATH**/ ?>