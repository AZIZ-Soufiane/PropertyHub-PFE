    ...
    <div class="w-full lg:w-1/3 border-r border-slate-100 flex flex-col bg-slate-50/30">
        ...
        <div class="overflow-y-auto flex-1">
            <?php $__empty_1 = true; $__currentLoopData = $conversations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $conv): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                ...
                ?>
                <a href="<?php echo e(route('admin.messages.show', $otherConv)); ?>"
                    class="flex items-center gap-x-4 p-4 hover:bg-white transition-all border-l-4 <?php echo e($isActive ? 'border-primary-500 bg-white' : 'border-transparent'); ?>">
                    <div class="size-10 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center font-bold flex-shrink-0">
                        <?php echo e(strtoupper(substr($otherConv->name ?? '?', 0, 1))); ?>

                    </div>
                    <div class="grow min-w-0">
                        <h3 class="font-bold text-sm text-slate-800 truncate"><?php echo e($otherConv->name ?? 'Unknown'); ?></h3>
                        <p class="text-xs text-slate-500 truncate font-medium <?php echo e($isActive ? 'text-primary-600' : ''); ?>"><?php echo e($msg ? $msg->content : 'Start a conversation'); ?></p>
                    </div>
                    <?php if($msg): ?>
