...
<div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition-all">
        ...
        <h3 class="text-3xl font-black text-gray-800"><?php echo e($stats['total_users'] ?? 0); ?></h3>
        <span class="flex items-center gap-x-1 text-emerald-600 font-bold text-sm mt-2">
            +<?php echo e($stats['new_users_this_month'] ?? 0); ?> this month
        </span>
    </div>
    ...
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition-all">
        ...
        <h3 class="text-3xl font-black text-gray-800"><?php echo e($stats['total_properties'] ?? 0); ?></h3>
        <span class="flex items-center gap-x-1 text-emerald-600 font-bold text-sm mt-2">
            +<?php echo e($stats['new_properties_this_month'] ?? 0); ?> this month
        </span>
    </div>
    ...
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition-all">
        ...
        <span class="text-gray-500 font-bold text-sm mt-2">
            <?php echo e($stats['pending_appointments'] ?? 0); ?> pending
        </span>
    </div>
    ...
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition-all">
        ...
        <h3 class="text-3xl font-black text-gray-800">$<?php echo e(number_format($stats['total_revenue'] ?? 0)); ?></h3>
...
<div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-3xl overflow-hidden">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <img class="w-10 h-10 rounded-full" src="<?php echo e($user->avatar ?? 'https://via.placeholder.com/40'); ?>" alt="">
                                <div>
                                    <p class="text-sm font-bold text-gray-800"><?php echo e($user->name); ?></p>
                                    <p class="text-xs text-gray-400 font-medium"><?php echo e($user->email); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="py-1 px-3 rounded-full text-[10px] font-bold uppercase tracking-wide
                                <?php echo e($user->role === 'admin' ? 'bg-blue-100 text-blue-700' : ($user->role === 'agent' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-700')); ?>">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="py-1 px-3 inline-flex items-center gap-1 rounded-full text-[10px] font-bold
                                <?php echo e($user->status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'); ?>">
                                <span class="w-1.5 h-1.5 rounded-full <?php echo e($user->status === 'active' ? 'bg-emerald-500' : 'bg-amber-500'); ?>"></span>
                                <?php echo e(ucfirst($user->status)); ?>

                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-400 font-semibold"><?php echo e($user->created_at->format('M d, Y')); ?></td>
    ...
    <?php if(isset($users) && $users->hasPages()): ?>
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            <p class="text-xs text-gray-400 font-semibold">Showing <?php echo e($users->firstItem()); ?> to <?php echo e($users->lastItem()); ?> of <?php echo e($users->total()); ?> users</p>
