...
<div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5">
        ...
        <div class="flex items-center gap-x-3">
            <h3 class="text-3xl font-black text-gray-800"><?php echo e($stats['active_listings'] ?? 0); ?></h3>
            <?php if(($stats['new_this_week'] ?? 0) > 0): ?>
                <span class="text-xs font-bold py-1 px-2 bg-emerald-100 text-emerald-800 rounded-lg">+<?php echo e($stats['new_this_week']); ?> new</span>
            <?php endif; ?>
        </div>
    </div>
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5">
        ...
        <div class="flex items-center gap-x-3">
            <h3 class="text-3xl font-black text-gray-800"><?php echo e($stats['pending_viewings'] ?? 0); ?></h3>
    ...
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5">
        ...
        <div class="flex items-center gap-x-3">
            <h3 class="text-3xl font-black text-gray-800"><?php echo e($stats['total_appointments'] ?? 0); ?></h3>
        </div>
    </div>
    <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5">
        <p class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Unread Messages</p>
        <div class="flex items-center gap-x-3">
            <h3 class="text-3xl font-black text-gray-800"><?php echo e($stats['unread_messages'] ?? 0); ?></h3>
        </div>
    </div>
</div>
...
<div class="grid lg:grid-cols-2 gap-8">
    ...
    <div class="space-y-4">
        <div class="flex justify-between items-center">
            ...
            <a href="<?php echo e(route('agent.appointments.index')); ?>" class="text-sm font-semibold text-blue-600 hover:text-blue-700">View Calendar</a>
        </div>
        ...
        <?php $__empty_1 = true; $__currentLoopData = $upcomingAppointments ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $appointment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="flex flex-col bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:border-blue-300 transition-all">
                <div class="flex justify-between items-start mb-4">
                    <div class="space-y-1">
                        <h3 class="font-bold text-gray-800 text-lg"><?php echo e($appointment->scheduled_at->format('D, M d')); ?></h3>
                        <p class="text-sm text-gray-500 font-medium"><?php echo e($appointment->time_slot); ?></p>
                    </div>
                    <span class="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs font-bold 
                        <?php echo e($appointment->status === 'confirmed' ? 'bg-emerald-100 text-emerald-800' : 'bg-blue-100 text-blue-800'); ?>">
                        <?php echo e(ucfirst($appointment->status)); ?>

                    </span>
                </div>
                <div class="mb-5 p-4 bg-gray-50 rounded-2xl">
                    <p class="text-sm text-gray-800 font-bold mb-1"><?php echo e($appointment->property->title); ?></p>
                ...
                <div class="flex gap-x-2">
                    <?php if($appointment->status === 'pending'): ?>
                        <form action="<?php echo e(route('agent.appointments.confirm', $appointment)); ?>" method="POST" class="flex-1">
                            <?php echo csrf_field(); ?>
                        ...
                        <form action="<?php echo e(route('agent.appointments.cancel', $appointment)); ?>" method="POST" class="flex-1">
                            <?php echo csrf_field(); ?>
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(route('agent.appointments.show', $appointment)); ?>" class="flex-1 py-2.5 px-4 text-sm font-bold rounded-xl border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 text-center transition-colors">
                            View Details
                        </a>
                    <?php endif; ?>
                </div>
    ...
    <div class="space-y-4">
        <div class="flex justify-between items-center">
            ...
            <a href="<?php echo e(route('agent.messages.index')); ?>" class="text-sm font-semibold text-blue-600 hover:text-blue-700">Open Inbox</a>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            <?php $__empty_1 = true; $__currentLoopData = $recentMessages ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <a href="<?php echo e(route('agent.messages.show', $message->sender)); ?>" 
                   class="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors group border-b border-gray-100 last:border-0">
                    ...
                    <div class="grow">
                        <div class="flex justify-between items-center">
                            <h4 class="text-sm font-bold text-gray-800"><?php echo e($message->sender->name); ?></h4>
                            <span class="text-[10px] font-medium text-gray-400"><?php echo e($message->created_at->diffForHumans()); ?></span>
                        </div>
                        <p class="text-xs text-gray-500 line-clamp-1"><?php echo e($message->content); ?></p>
                    </div>
                </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <img class="w-10 h-10 rounded-xl object-cover" 
                                     src="<?php echo e($property->images->first()?->first_url ?? 'https://via.placeholder.com/80'); ?>" alt="">
                                <div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="py-1 px-3 inline-flex items-center gap-1 rounded-full text-[10px] font-bold 
                                <?php echo e($property->status === 'approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'); ?>">
                                <span class="w-1.5 h-1.5 rounded-full <?php echo e($property->status === 'approved' ? 'bg-emerald-500' : 'bg-amber-500'); ?>"></span>
...
<?php echo $__env->make('layouts.agent', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\GitHub\PropertyHub-PFE\PropertyHub-PFE\PropertyHub\resources\views/agent/dashboard.blade.php ENDPATH**/ ?>